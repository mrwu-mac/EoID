...
class DynamicBCELoss(nn.Module):
    ...
    def __init__(self, zero_shot=None, reduction='mean', loss_weight=1):
        ...
        self.unseen_mask = torch.tensor([1. for i in range(117)])
...
class HOIModel(nn.Module):
    ...
    def __init__(self, backbone, transformer, num_obj_classes, num_verb_classes, num_queries, aux_loss=False,
        ...
        self.verb_class_embed = nn.Linear(hidden_dim, 1024)
        ...
        self.semblock = SemanticBlock('a_emb', dims=[1024, 4096, 4096, 1024], graph='consistency_graph.pkl',
                                      msg_pass_cfg=dict(type='GAT'), heads=[8, 8, 4], )
    ...
    def forward(self, samples: NestedTensor):
        ...
        text_features = self.semblock()['a_emb']
        text_features = text_features / text_features.norm(dim=-1, keepdim=True)
        outputs_verb_class = outputs_verb_class @ text_features.t()
        outputs_verb_class = outputs_verb_class * self.score_factor

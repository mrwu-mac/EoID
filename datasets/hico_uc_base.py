...
from .static_hico import UC_HOI_IDX, HICO_INTERACTIONS, HOI_IDX_TO_ACT_IDX, HOI_IDX_TO_OBJ_IDX, OBJ_IDX_TO_OBJ_NAME, \
    ACT_IDX_TO_ACT_NAME


class HICODetection(torch.utils.data.Dataset):
    """
    Base HICO UC setting, delete all unseen label.
    """
            ...
            for idx, img_anno in enumerate(self.annotations):
                for hoi in img_anno['hoi_annotation']:
                    if hoi['subject_id'] >= len(img_anno['annotations']) or hoi['object_id'] >= len(
                        ...
                        break
                    # if self._valid_verb_ids.index(hoi['category_id']) in ua:
                    #     break
                else:
                    self.ids.append(idx)
        else:
        ...
        if self.img_set == 'train':
            ...
            for hoi in img_anno['hoi_annotation']:
                ...
                if sub_obj_pair in sub_obj_pairs:
                    obj_label = obj_labels[sub_obj_pairs.index(sub_obj_pair)]
                    verb = self._valid_verb_ids.index(hoi['category_id'])
                    combi = (verb, obj_label)
                    if not combi in self.uc:
                        verb_labels[sub_obj_pairs.index(sub_obj_pair)][verb] = 1
                    else:
                        st[sub_obj_pairs.index(sub_obj_pair)] = 1
                else:
    ...
    def get_uc(self):
        uc = [(HOI_IDX_TO_ACT_IDX[idx], HOI_IDX_TO_OBJ_IDX[idx]) for idx in UC_HOI_IDX[self.uc_type]]
        return uc

    def set_uc_hois(self):
        self.seen_triplets = []
        ...
        for hoi in HICO_INTERACTIONS:
            ...
            if hoi['interaction_id'] in UC_HOI_IDX[self.uc_type]:

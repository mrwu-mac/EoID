...
class HICOUAEvaluator():
    def __init__(self, preds, gts, rare_triplets, non_rare_triplets, correct_mat, args):
        ...
        for i, img_gts in enumerate(gts):
            ...
            self.gts.append({
                'filename': filename,
                'annotations': [{'bbox': list(bbox), 'category_id': label} for bbox, label in
    ...
    def compute_map(self):
        ...
        seen_recall = defaultdict(lambda: 0)
        unseen_recall = defaultdict(lambda: 0)
        for triplet in self.gt_triplets:
            ...
            if triplet in self.rare_triplets:
                ...
                seen_recall[triplet] = max_recall[triplet]
            elif triplet in self.non_rare_triplets:
                non_rare_ap[triplet] = ap[triplet]
                unseen_recall[triplet] = max_recall[triplet]
            else:
                print('Warning: triplet {} is neither in rare triplets nor in non-rare triplets'.format(triplet))
        ...
        m_recall_seen = np.mean(list(seen_recall.values()))
        m_recall_unseen = np.mean(list(unseen_recall.values()))

        print('--------------------')
        print(
            'mAP: {} mAP seen: {}  mAP unseen: {}  mean max recall: {}  mean recall seen: {}  mean recall unseen: {}'.format(
                m_ap, m_ap_rare, m_ap_non_rare, m_max_recall, m_recall_seen, m_recall_unseen))
        print('--------------------')

        return {'mAP': m_ap, 'mAP seen': m_ap_rare, 'mAP unseen': m_ap_non_rare, 'mean max recall': m_max_recall,
                'mean recall seen': m_recall_seen, 'mean recall unseen': m_recall_unseen}
                    ...
                    for gt_hoi in gt_hois:
                        if gt_hoi['subject_id'] in pred_sub_ids and gt_hoi['object_id'] in pred_obj_ids \
                            ...
                            if min_overlap_gt > max_overlap:
                                ...
                                max_gt_hoi = gt_hoi
                triplet = (
                pred_bboxes[pred_hoi['subject_id']]['category_id'], pred_bboxes[pred_hoi['object_id']]['category_id'],
